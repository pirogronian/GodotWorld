...
func test_empty_registers():
	var d1 = Device.new()
	assert_true(d1.register_read(Device.TransportType.Pipe, 0, "some_register") == null)
	assert_true(d1.add_slot(Device.TransportType.Pipe, false))
	assert_true(d1.register_read(Device.TransportType.Pipe, 0, "some_register") == null)
	assert_true(d1.add_slot(Device.TransportType.Pipe, true))
	assert_true(d1.register_read(Device.TransportType.Pipe, 1, "some_register") == null)
	d1.queue_free()
...
func test_device_registers():
	...
	assert_eq(td.register_read(t_g, 0, "Reg1"), null)
	assert_eq(td.register_read(t_p, 0, "Reg1"), null)
	assert_eq(td.register_read(t_p, 1, "Reg1"), 1)
	assert_eq(td.register_read(t_w, 0, "Reg1"), 1)
	assert_eq(td.register_read(t_w, 1, "Reg1"), null)
	assert_eq(td.register_read(t_f, 0, "Reg1"), 1)
	assert_false(td.register_write(t_g, 0, "Reg1", 2))
	assert_true(td.register_write(t_f, 0, "Reg1", 2))
	assert_eq(td.register_read(t_g, 0, "Reg1"), null)
	assert_eq(td.register_read(t_p, 0, "Reg1"), null)
	assert_eq(td.register_read(t_p, 1, "Reg1"), 2)
	assert_eq(td.register_read(t_w, 0, "Reg1"), 2)
	assert_eq(td.register_read(t_w, 1, "Reg1"), null)
	assert_eq(td.register_read(t_f, 0, "Reg1"), 2)
